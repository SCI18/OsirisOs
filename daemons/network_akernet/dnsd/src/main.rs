// dnsd — Daemon #24, NetworkAkernet
// DNS filtering, AdGuardHome integration
// "The query is weighed. The answer is filtered."
...
impl Dnsd {
    ...
    async fn connect_to_bridge(&self) -> Result<mpsc::Receiver<ReaderEvent>> {
        ...
        *self.socket_write_half.lock().await = Some(write_half);
        info!("[dnsd] Connected to AkerNet Bridge at {}", SOCKET_PATH);
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                match reader.read_line(&mut line).await {
                    ...
                    Err(e) => {
                        debug!("[dnsd] Socket read error: {}", e);
    ...
    async fn handle_bridge_message(&self, msg: BridgeMessage) -> Result<()> {
        match msg {
            BridgeMessage::Acknowledged { name } => {
                info!("[dnsd] Registration acknowledged by Bridge: {}", name);
            }
            BridgeMessage::RegistrationRejected { name, reason } => {
                error!("[dnsd] Registration rejected: {} — {}", name, reason);
                std::process::exit(1);
            }
            ...
            BridgeMessage::Stop => {
                info!("[dnsd] Received Stop from Bridge, shutting down");
                self.shutdown().await?;
                std::process::exit(0);
            }
            BridgeMessage::Reload => {
                info!("[dnsd] Received Reload from Bridge");
            }
            BridgeMessage::Restart => {
                info!("[dnsd] Received Restart from Bridge");
                self.shutdown().await?;
                std::process::exit(0);
            }
            BridgeMessage::Forward(_) => {
                debug!("[dnsd] Received unexpected Forward message, ignoring");
            }
        }
        Ok(())
    }
    ...
    async fn shutdown(&self) -> Result<()> {
        info!("[dnsd] Shutting down gracefully");
    ...
    async fn emit_alert(&self, alert_key: &str, severity: AlertSeverity, payload: Value) -> Result<()> {
        ...
        self.send_frame(msg).await?;
        info!("[dnsd] Alert emitted: {} [{:?}]", alert_key, severity);
        Ok(())
    }

    async fn collect_and_check(&self) -> Result<()> {
        // TODO: Implement DNS monitoring
        // - Query AdGuardHome API for stats (blocked queries, upstream latency)
        // - Monitor upstream DNS health (latency, failures)
        // - Track cache hit/miss ratios
        // - Alert on upstream DNS failures, high latency, cache poisoning attempts
        // - Manage blocklist updates and reload triggers
...
async fn main() -> Result<()> {
    ...
    info!("[dnsd] Osiris DNS Daemon v{} starting", VERSION);

    let dnsd = Dnsd::new().await?;
    let mut reader_rx = dnsd.connect_to_bridge().await?;
    dnsd.register().await?;
        ...
        while let Some(event) = reader_rx.recv().await {
            match event {
                ReaderEvent::Bridge(msg) => {
                    ...
                    if let Err(e) = dnsd.handle_bridge_message(msg).await {
                        debug!("[dnsd] Bridge message handler error: {}", e);
                    }
                    if is_ack { break; }
                }
    ...
    info!("[dnsd] Registration complete, entering monitoring loop");
    ...
    loop {
        tokio::select! {
            _ = poll_interval.tick() => {
                if let Err(e) = dnsd.collect_and_check().await {
                    error!("[dnsd] Collection error: {}", e);
                }
            }
            ...
            event = reader_rx.recv() => {
                match event {
                    ...
                    Some(ReaderEvent::Closed) | None => {
                        warn!("[dnsd] Bridge connection closed, exiting");
                        break;
                    }
                }
            }
